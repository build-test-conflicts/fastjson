...
public class JavaBeanDeserializer implements ObjectDeserializer {
  private FieldDeserializer[] fieldDeserializers;
  FieldDeserializer[] sortedFieldDeserializers;
  Class<?> clazz;
  public JavaBeanInfo beanInfo;
  private ConcurrentMap<String,Object> extraFieldDeserializers;
  private Map<String,FieldDeserializer> alterNameFieldDeserializers;
  ...
  public FieldDeserializer getFieldDeserializer(  long hash){
    ...
    if (hashArrayMapping == null) {
      ...
      for (int i=0; i < sortedFieldDeserializers.length; i++) {
        int p=Arrays.binarySearch(hashArray,TypeUtils.fnv1a_64(sortedFieldDeserializers[i].fieldInfo.name));
        if (p >= 0) {
          mapping[p]=(short)i;
        }
      }
  ...
  @SuppressWarnings({"unchecked"}) public <T>T deserialzeArrayMapping(  DefaultJSONParser parser,  Type type,  Object fieldName,  Object object){
  ...
  @SuppressWarnings({"unchecked","rawtypes"}) <T>T deserialze(  DefaultJSONParser parser,  Type type,  Object fieldName,  Object object,  int features,  int[] setFlags){
    ...
    try {
      ...
      if (token != JSONToken.LBRACE && token != JSONToken.COMMA) {
        ...
        StringBuilder buf=(new StringBuilder()).append("syntax error, expect {, actual ").append(lexer.tokenName()).append(", pos ").append(lexer.pos());
      ...
      for (int fieldIndex=0, notMatchCount=0; ; fieldIndex++) {
        ...
        if (matchField) {
          ...
          boolean match=parseField(parser,key,object,type,fieldValues == null ? new HashMap<String,Object>(this.fieldDeserializers.length) : fieldValues,setFlags);
          if (!match) {
            if (lexer.token() == JSONToken.RBRACE) {
              lexer.nextToken();
              break;
      ...
      if (object == null) {
        ...
        if (beanInfo.creatorConstructor != null) {
          ...
          try {
            ...
            throw new JSONException("create instance error, " + paramNames + ", "+ beanInfo.creatorConstructor.toGenericString(),e);
          }
          if (paramNames != null) {
  ...
  @SuppressWarnings({"unchecked","rawtypes"}) protected static void parseArray(  Collection collection,  ObjectDeserializer deser,  DefaultJSONParser parser,  Type type,  Object fieldName){
    final JSONLexerBase lexer=(JSONLexerBase)parser.lexer;
    int token=lexer.token();
    if (token == JSONToken.NULL) {
      lexer.nextToken(JSONToken.COMMA);
